...
from agents.tools.templates import order_draft_template, vn_order_draft_template, MULTI_MESSAGE_DELIMITER
...
class ADKHelper:
    ...
    def set_name_change_state(self, user_id: str, pending: bool) -> None:
        """Set or clear the 'awaiting new name' flag on the user doc."""
        try:
            self._user_root(user_id).set({"name_change_pending": pending}, merge=True)
        except Exception as e:
    ...
    def _twilio_send_media(self, to_number: str, media_url: str, *, body: str = "", content_type: str = "") -> bool:
        """
        ...
        try:
            ...
            if 200 <= r.status_code < 300:
                logger.info("twilio.media.sent", to=to_number, content_type=content_type)
                return True
    ...
    async def _call_agent_text_only(self, query: str, user_id: str, session_id: str) -> str:
        ...
        try:
            pending_rec_ctx = self._build_pending_recommendations_context(user_id)
        except Exception:
            pending_rec_ctx = ""
        if pending_rec_ctx:
            merged_query = (
        ...
        try:
            forced = adk_guardrails.pop_forced_reply()
            ...
            if forced:
                ...
                try:
                    ...
                    pass
                try:
                    adk_guardrails.remember_forced_reply(None)
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def _sanitize_customer_visible_text(self, text: Any, *, user_id: str) -> str:
        ...
        if self._looks_like_internal_error_reply(candidate):
            ...
            return CUSTOMER_SAFE_FALLBACK_TEXT
        return self._normalize_order_unit_terms(candidate)
