...
def greeting_template(
    ...
    Malaysian Chinese version, Xiao Li persona.
...
def manual_order_template(args: dict) -> str:
    ...
    if sku:
        ...
        if price is not None:
            output.append(f"好的，*{name}* 的价格是 *RM {price}*。")
        else:
    ...
    if promo_applied:
        ...
        if old_total is not None and new_total is not None:
            output.append(f"之前是 ~RM {old_total}~，现在是 *RM {new_total}*。")
        if savings is not None:
            output.append(f"等于省 *RM {savings}*。")
    ...
    if promo_needs_more:
        ...
        if old_bulk_total is not None and new_bulk_total is not None:
            output.append(f"总额从 ~RM {old_bulk_total}~ 变成 *RM {new_bulk_total}*。")
        if save_bulk is not None:
            output.append(f"相当于再省 *RM {save_bulk}*。")
    ...
    if no_promo:
        ...
        if total is not None:
            output.append(f"好的，目前总额是 *RM {total}*。")
...
def order_draft_template(
    ...
    *,
    ...
    #         out.append(
    #             f"卖完后，销售大约 *{total_sell} RM*，"
    #             f"利润大约 *{profit} RM* (~{margin_pct:.1f}% 利润率)。"
    #         )
    #     else:
    #         out.append(
    #             f"卖完后，销售大约 *{total_sell} RM*，"
    ...
    if show_breakdown:
        ...
        if subtotal_breakdown is not None:
            out.append(f"小计: RM {subtotal_breakdown:,.2f}")
        if discount_breakdown is not None:
            out.append(f"总折扣: RM {discount_breakdown:,.2f}")
        # if total_breakdown is not None:
        #     out.append(f"总额:         RM {total_breakdown:,.2f}")
        ...
        #     out.append(f"利润:         RM {profit_breakdown:,.2f}")
        # # if profit_total_breakdown is not None:
        # #     out.append(f"利润总额:     RM {profit_total_breakdown:,.2f}")
        # margin_display = (
        #     profit_margin_pct_breakdown
...
def vn_order_draft_template(args: dict) -> str:
    ...
    - Malaysian Chinese spoken style.
    ...
    for line in lines:
        ...
        if line_total is not None:
            parts.append(f"{name}，总共 {qty_str} {unit_word}，{line_total} Ringgit")
        else:
            parts.append(f"{name}，总共 {qty_str} {unit_word}")

    if total is not None:
        parts.append(f"总额是 {total} Ringgit。")
    ...
    if total_sell is not None and profit is not None:
        parts.append(
            f"如果全部卖完，总销售大概 {total_sell} Ringgit，"
            f"利润大概 {profit} Ringgit。"
        )
    else:
        parts.append("利润应该不错，InshaAllah。")
...
def invoice_status_template(args: dict) -> str:
    """
    Invoice / order status summary in Malaysian Chinese.
    """
    company_name = _resolve_company_name(args)
    ...
    if old_total:
        lines.append(f"• 之前金额：~RM {old_total}~")

    lines.append(f"• 最终总额：*RM {total}*")
    ...
    if total_sell is not None and profit is not None:
        if margin_pct is not None:
            lines.append(
                f"• 预计销售约 *RM {total_sell}*，利润约 *RM {profit}* "
                f"(~{margin_pct:.1f}% 利润率)。"
            )
        else:
            lines.append(
    ...
    if suggest_topup and topup_gap:
        lines.append(
            f"老板，如果再加大约 RM {topup_gap} 的货，组合会更好... "
            "要不要看看小SKU？"
        )
...
def reorder_template(args: dict) -> str:
    """
    Shows last order for quick repeat in Malaysian Chinese.
    """
    company_name = _resolve_company_name(args)
...
def bundles_template():
    """
    Static-ish bundles message in Malaysian Chinese.
    """
    company_name = _resolve_company_name()
    intro_options = [
...
def product_info_template(args: dict) -> str:
    """
    Product info for SKUs in Malaysian Chinese.
    """
    sku = args.get("sku") or {}
...
def recommendations_template(args: dict) -> str:
    """
    Top sellers / upsell list in Malaysian Chinese.
    """
    company_name = _resolve_company_name(args)
    intro_pool = [
...
def seasonal_advice_template():
    """
    Simple seasonal nudge in Malaysian Chinese.
    """
    company_name = _resolve_company_name()
    season_lines = [
...
def current_total_reminder(total: float, casual: bool = True) -> str:
    """
    Quick reminder of current order total in Malaysian Chinese.
    """
    if casual:
        phrases = [
            f"目前订单在 RM {total}",
        ...
        return f"当前订单总额：RM {total}"


# 12) Objection Handling (simple, no-arg)
def objection_handling_template():
    """
    Generic objection handling lines for Xiao Li, Malaysian Chinese.
    """
    blocks = []
